...
The ignored root-level `config.py` module mirrors the machine-specific locations in `user-paths.md` for repository Python scripts, including the static vcpkg installation used by the build. Copy `config.example.py` to `config.py`, fill in the local paths, and run scripts from the repository root. Import paths as `from config import DLL_DEPLOY, MO2_PROFILE, VCPKG_STATIC_DIR`; update both ignored files when a local path changes.
